...
void Fruit::roleFruit(Serpent & s, Terrain & t, Extra & e) // Determine l'effet de chaque fruit sur le Serpent 1
{
    ...
    else if(f=='8') {t.setChoix(3),s.setX(1),s.setY(1),s.setDirection(0);} //on change de terrain, position et direction du serpent reinitialiser
}
...
{
    srand(time(NULL));
    unsigned int etat=rand()%6; // Tire un chiffre aleatoirement entre 0 et 6
